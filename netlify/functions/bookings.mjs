...
export default async (req) => {
    ...
    try {
        if (req.method === 'POST') {
            const { name, email, room, checkin, checkout, price, status } = await req.json();

            const result = await sql`
        INSERT INTO bookings (name, email, room, checkin, checkout, price, status)
        VALUES (${name}, ${email}, ${room}, ${checkin}, ${checkout}, ${price}, ${status || 'Confirmed'})
        RETURNING *
      `;
